...
class samm_c3_DataSet(data.Dataset):
    def __init__(self, raf_path, phase,num_loso, transform = None, basic_aug = False, transform_norm=None):
        ...
        df = pd.read_excel(os.path.join(self.raf_path, 'SAMM_136.xlsx'),usecols=[0,1,3,4,5,8,9])
    ...
    def __getitem__(self, idx):
        ...
        path_on0 = os.path.join(self.raf_path, 'Cropped/', '%03d' % int(sub), f, on0)

        path_apex0 = os.path.join(self.raf_path, 'Cropped/', '%03d' % int(sub), f, apex0)
