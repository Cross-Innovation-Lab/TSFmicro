...
from base import casme3_difference


class cas3_c4_DataSet(data.Dataset):
    def __init__(self, raf_path, phase,num_loso, transform = None, basic_aug = False, transform_norm=None):
        ...
        df = pd.read_excel(r'C:\Users\12171\Desktop\casme3.xlsx', usecols=[0,1,2,3,4,5,7])
        ...
        for (f,sub,onset,apex,offset,label_all,label_au) in zip(File_names,Subject,Onset_num,Apex_num,Offset_num,Label_all,Label_au):
            ...
            if label_all == 'anger' or label_all == 'disgust' or label_all == 'fear' or label_all == 'happy' or label_all == 'sad' or label_all == 'surprise' or label_all == 'others':
    ...
    def __getitem__(self, idx):
        ...
        if self.phase == 'train':
            ...
            on0, apex0 = casme3_difference(int(onset), int(apex), int(offset), self.raf_path, sub, f)
            on0 = str(on0)
            apex0 = str(apex0)
