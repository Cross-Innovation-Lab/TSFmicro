...
# @Author : BingYu Nan
# @Location : Wuxi
# @Time : 2024/10/18 19:13
# -*- coding: utf-8 -*-
# @Author : BingYu Nan
# @Location : Wuxi
# @Time : 2024/10/18 19:01
import torch
...
class fusion_before_VisRetNet(nn.Module):
...
class fusion_after_VisRetNet(nn.Module):
...
class res_VisRetNet(nn.Module):
    ...
    def forward_features(self, x, y=None, res_y=None):
        x = self.patch_embed(x)
    ...
    def forward(self, x, y=None, res_y=None):
        x = self.forward_features(x, y, res_y)
        x = self.head(x)
        return x

@register_model
def fusion_before_rmt(num_class):
    model = fusion_before_VisRetNet(
        embed_dims=[64, 128, 256, 512],
        depths=[1, 1, 2, 1],
        num_heads=[2, 2, 2, 2],
        init_values=[2, 2, 2, 2],
...
@register_model
def fusion_after_rmt(num_class):
    model = fusion_after_VisRetNet(
        embed_dims=[64, 128, 256, 512],
        depths=[1, 1, 2, 1],
        num_heads=[2, 2, 2, 2],
        init_values=[2, 2, 2, 2],
